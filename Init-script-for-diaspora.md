...
function wait_for_services_to_start {
    ...
    while [ $STARTED_SERVICES -lt 3 ]; do
        ...
        if [ $WEBSOCKET_STARTED = 0 ]; then
            if status_of_proc -p log/diaspora-wsd.pid diaspora > /dev/null; then
...
function start_services {
    ...
    if [ "$(./script/get_config.rb enable_thin script_server)" = "true" ]; then
        echo "Thin"
        bundle exec thin start -d --pid log/thin.pid $thin_values > log/thin.log &
    fi
}
...
function stop_services 
{
    ...
    if [ -e log/thin.pid ]; then
        bundle exec thin --pid log/thin.pid stop > /dev/null
    else
    ...
    echo "Websocket"
    if [ -e log/diaspora-wsd.pid ]; then
        WEBSOCKET_PID=`cat log/diaspora-wsd.pid`
