...
Diaspora needs to know what host it's running on. Copy config/app.yml.example
to config/app.yml, put your external url into the pod_url field, and make any other
...
* Edit rails_env in the script_server section in config/app.yml
...
Just run `./script/server`. This will start thin, redis, a resque worker and the websocket server. The application is then available at http://your_pod:3000. You can change port by editing thin_port in the script_server section in config/app.yml or setup a reverse proxy (google it ;)) if you want to run Diaspora for example at a subdomain or use https more easily.
