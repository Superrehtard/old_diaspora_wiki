If you're a developer who wants to work on the Diaspora source code and submit your changes for consideration to be merged into the master branch, here's how.  Thanks to Thinkup for their awesome developer guide.

h1. Quickfire Do's and Don't's

p. If you're familiar with git and GitHub, here's the short version of what you need to know. Once you fork and download the Diaspora:
...
p. A visual representation of what a Diaspora contributor's GitHub/git workflow should look like. (Click to enlarge.)

!http://farm5.static.flickr.com/4012/4702060815_a1a7357768.jpg!:http://www.flickr.com/photos/ginatrapani/4702060815/sizes/l/

h3. Step-by-step (the short version)
...
# Branch for new issue ($ git checkout -b 100-new-feature)
...
# Repeat steps 5-9 till dev is complete
# Push branch to GitHub ($ git push origin 100-new-feature)
# Issue pull request (Click Pull Request button) 

p. If you're new to git and GitHub, here's the longer version of these instructions.

h1. Fork the Repository to Contribute Code

p. Here's how to fork the Diaspora to begin working on it.

p. Create an account on GitHub and establish connectivity between your GitHub account and your hosting server.

# Create a free account on GitHub. 
# Fork the project from "diaspora/diaspora":http://github.com/diaspora/diaspora 
# Make sure you've got an SSH public key created on your server and recorded in your GitHub account. You can see your SSH Public Keys on the Account Overview section of your github account. Here's a "good guide":http://github.com/guides/providing-your-ssh-key.
...
p. Clone your GitHub fork to your development machine and set up diaspora.

# Run a clone command against your github fork. It will look something like this except that it will use your GitHub account name:  
...
p. That will download all your forked GitHub files to a git repository on your development server. If you have problems, check the permissions on the newly created diaspora directory on your server.

p. Refer to the guide to running Diaspora linklinklinklinklink to get it working on your machine.

h2. Create an Issue-Specific Development Branch

p. Before you start working on a new feature or bugfix, create a new branch dedicated to that one change named by issue number and description. If you're working on Issue #359, a aspect naming bugfix, create a new branch with the issue number and description, like this:
...
p. Edit and test the files on your development server. When you've got something the way you want and established that it works, commit the changes to your branch on your development server's git repo. 
...
p. *IMPORTANT*: Before you issue a pull request, make sure it gets accepted by running through the [[Developer Guide: Pull Request Checklist]] first.
...
h2. Squash Many Commits Related to a Single Issue into a Few Commits (Probably one)

p. Once you have rebased your work on top of the latest state of the upstream master, you may have many commits related to the issue you were working on. Once everything is done, squash them into a single commit with a descriptive message, like "Issue #359: Unique Aspect Names."  If your pull request is very complicated, feel free to subit a pull request with several commits in it.

p. To squash four commits into one, do the following:
...
p. In the text editor that comes up, replace the words "pick" with "squash" next to the commits you want to squash into the commit before it. Save and close the editor, and git will combine the "squash"'ed commits with the one before it. Git will then give you the opportunity to change your commit message to something like, "Issue #359: Unique Aspect Names."

p. %{color:red}Important: If you've already pushed commits to GitHub, and then squash them locally, you will not be able to push that same branch to GitHub again.% Create a new branch--like 359-aspect-names-squashed or 359-aspect-names-rc1 (for release candidate 1)--and squash your commits there. Once everything is squashed and ready, push the new squashed branch to GitHub and send your pull request to Raphael.
